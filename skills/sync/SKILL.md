...
1. `CLAUDE.md` — current priorities, active plays, team focus
...
5. All files in `outputs/plays/` — active plays and their results tables
...
| Signal performance log | A play has been live for 14+ days with no results recorded |
| Play results table | Play is live but results rows are empty or older than 7 days |
...
- Which plays are currently live (from `outputs/plays/`)
...
- Any plays pausing or wrapping up?

### 2b. Signal Performance Log

If play metrics are available, for each active signal:
- Count sends, replies, and meetings booked from the play results table
- Calculate reply rate and meeting rate
- Draft an updated row if the numbers changed

If metrics aren't in the repo yet, ask:

```
What were the results on active plays this week?
Paste: Play name | Sends | Replies | Meetings booked
```

Flag: if a signal has 30+ sends with no meetings booked, note it explicitly.

### 2c. Active Play Results

For each play in `outputs/plays/` with a live results table:
...
How to re-upload:
1. Open your [Company] GTM Assistant Project in Claude.ai
